...
python3 {SKILL_DIR}/assets/build_viewer.py -d YYYY-MM-DD -f 出发站 -t 到达站 --hubs 徐州,商丘 -o ~/Desktop/result.html
```

功能：表头排序（全程△/等待△/费用△/出发△/到达△ ▲▼）、多维过滤（出发/到达时间、起点站、终点站、中转站、换乘类型、车种、费用 ≤、车次1席位、车次2席位）、每程显示发时→到时防跨日买错、拖拽JSON文件加载。

## 技术架构

- **并发**：`ThreadPoolExecutor` 5路并行，每线程独立`Session` + 随机UA
- **防限流**：8条UA池 (macOS/Win/Linux/iOS/Android × Chrome/Safari/Firefox/Edge) 轮换 + 请求50-200ms随机抖动
...
- **容错**：`_get`×3 → `query_tickets`×2 → 枢纽空换Session → CLI刷session = 四层兜底
